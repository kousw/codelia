...
Desktop-capability-gated tools may be appended at agent creation time without prompt changes; the first shipped case is `ui_render`, exposed only when `ui_capabilities.supports_generated_ui` is present from the client initialize handshake.
...
- Desktop-only tools should follow the same rule: prefer capability gating plus precise tool description/schema over desktop-specific prompt text. If a tool only exists for one UI client (for example `ui_render`), the description must say when to use it, when not to use it, and what bounded shape it supports.
- `ui_render` should advertise that it can express compact charts/diagrams and class-diagram-ish architecture maps in addition to tables and summaries, but it still remains bounded by the allow-listed node catalog and must not imply arbitrary HTML/JS execution.
