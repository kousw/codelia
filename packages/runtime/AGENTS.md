...
For Anthropic, runtime resolves `max_tokens` from model metadata limits (`max_output_tokens` -> `max_input_tokens` -> `context_window`) with static registry fallback and guarantees it stays above legacy extended `thinking.budget_tokens` when applicable. Opus 4.7/4.8 keep provider-native `xhigh` and `max` distinct; Opus 4.6 and Sonnet 4.6 support adaptive-thinking `max` but fall canonical `xhigh` back to `high`; Opus 4.5 uses manual extended thinking and falls canonical `max` back to `xhigh` without `output_config.effort`.
