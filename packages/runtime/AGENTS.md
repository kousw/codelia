...
Agent-facing `shell` / `shell_status` / `shell_wait` / `shell_result` / `shell_cancel` return compact JSON payloads instead of full task dumps; terminal results include only one selected output field (`output` on success or `error_output` on failure) plus `cache_id` when cache-backed, while live/explicit log reads stay on `shell_logs`.
