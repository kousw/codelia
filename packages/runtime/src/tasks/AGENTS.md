...
- In `shell-executor.ts`, omitted `timeoutSeconds` means no execution timer is armed; foreground callers are responsible for supplying the default/capped timeout when they need one, and explicit timeout values must stay within Node's timer range (`2147483s` max).
