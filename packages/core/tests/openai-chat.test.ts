...
describe("ChatOpenAI websocket mode", () => {
	...
	test("extends websocket response timeout when events continue to arrive", async () => {
			...
			const interval = setInterval(() => {
				...
				if (tick >= 5) {
					...
					ws.emit("response.completed", {
						type: "response.completed",
						sequence_number: 1,
						response: buildWsResponse("resp_ws_idle_extended"),
					});
				}
			}, 12);
		};
		...
		const completion = await chat.ainvoke(
			{ messages: [{ role: "user", content: "keep ws alive with activity" }] },
			{ sessionKey: "session-ws-idle-extend-1" },
		);

		expect(completion.provider_meta).toEqual({
			response_id: "resp_ws_idle_extended",
			transport: "ws_mode",
			websocket_mode: "on",
			fallback_used: false,
			chain_reset: true,
	...
	test("falls back to http in websocket_mode=auto when ws response idle timeout is hit", async () => {
		...
		const completion = await chat.ainvoke(
			{ messages: [{ role: "user", content: "ws idle timeout fallback" }] },
			{ sessionKey: "session-ws-idle-timeout-1" },
		);
	...
	test("falls back to http in websocket_mode=auto when ws connect timeout is hit", async () => {
		const calls: StreamCall[] = [];
		const mockClient = {
			responses: {
				stream: (
		...
		const chat = new ChatOpenAI({
			...
			websocketResponseIdleTimeoutMs: 200,
			createResponsesWs: () => ws,
		});

		const completion = await chat.ainvoke(
			{ messages: [{ role: "user", content: "ws connect timeout fallback" }] },
			{ sessionKey: "session-ws-connect-timeout-1" },
		);
