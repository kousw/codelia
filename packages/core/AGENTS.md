...
The shared system prompt mission now leads with solving the user's task, producing the required artifact/behavior, and continuing until the concrete success criterion is met, rather than framing the agent primarily as a helper for shipping changes.
...
The shared system prompt now makes non-trivial planning goal-first: first survey the whole task to form a blueprint, then identify the required final artifact/behavior, key constraints, path to the goal, critical points, success criterion, and strongest feasible local verification before iterating, and do not mark plan steps complete when only a weaker proxy has passed.
