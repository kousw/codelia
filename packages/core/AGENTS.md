...
The shared system prompt now also teaches shell/task execution discipline with explicit tool naming: use `shell` to start shell work, prefer `background=true` for uncertain or long-running commands, use `shell_list` to rediscover active tasks, and use `shell_status`/`shell_logs`/`shell_wait`/`shell_result`/`shell_cancel` with the returned stable `key` to monitor retained shell tasks explicitly instead of fire-and-forget.
