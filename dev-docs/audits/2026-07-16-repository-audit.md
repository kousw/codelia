...
- Audit status: completed. The user-selected low-risk remediation scope completed on 2026-07-16 resolved 6 findings and partially remediated 4. The 2026-07-18 follow-ups resolved AUD-005, AUD-009, and AUD-033. The 2026-08-01 provider-policy implementation resolved AUD-018 and partially remediated AUD-035, bringing the total to 10 resolved findings and 6 partially remediated findings; the remaining findings stay open.
...
| AUD-018 | **Resolved 2026-08-01:** Public LLM retry options were no-ops | `Agent` now normalizes the public retry/window options and routes interactive model steps through the shared abortable coordinator. Deterministic tests cover attempts, wait hints, cancellation, retry-window expiry, and progress events. |
...
| AUD-035 | **Partially remediated 2026-08-01:** Provider retry, timeout, progress, and error-redaction behavior was inconsistent | Core now owns bounded abortable retry for interactive model steps, emits `llm.retry`, caps provider wait hints at 60 seconds, applies a 20-minute window to retry waits and subsequent attempts without aborting an active request, and classifies provider failures before retry. Provider transports retain ownership of active request/progress timeouts. Codelia-constructed SDK-backed provider clients use `maxRetries: 0`. Moonshot distinguishes TPD/quota from short-window limits and forbids replay after buffered SSE output. Runtime persists safe structured provider errors without raw messages/stacks, and TUI renders retry progress. Remaining: separate first-byte/stream-idle watchdogs, stream-first internal events, buffered-delivery guards for the other streaming adapters, helper-call retry visibility, and attempt timing diagnostics. See [`retry-and-failures.md`](../specs/providers/retry-and-failures.md). |
...
2. **Provider availability and error hygiene:** remaining AUD-035 work; add first-byte/stream-idle watchdogs, finish buffered-delivery guards, and expose attempt timing diagnostics.
...
7. **Test, release, and maintenance cleanup:** AUD-021, remaining AUD-023 coverage, AUD-026, AUD-029 recurrence prevention, AUD-030, AUD-032, and the remaining AUD-034 transitive warning.
