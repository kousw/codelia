...
use crate::app::{
    AppState, ContextPanelState, ModelListMode, ModelListPanelState, ModelPickerState,
...
fn build_model_list_panel(
    ...
    for model in models {
        ...
        rows.push((model, ctx, input, output, is_current));
    ...
    let rendered_rows = rows
        .into_iter()
        .map(|(model, ctx, input, output, is_current)| {
